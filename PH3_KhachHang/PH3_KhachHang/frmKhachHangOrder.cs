...
{
    public partial class frmKhachHangOrder : Form
    {
        XyLyDatabase db = new XyLyDatabase();
        private string currentMaPH;
        ...
        {
            string sql = string.Format(@"select * from THUEPHONG where MaPH='PH_10' and TinhTrang=N'Đã Nhận Phòng'", MaPH);
        ...
        private void frmKhachHangOrder_Load(object sender, EventArgs e)
        {
            // Hiển thị danh sách các phòng đã có sẵn
            LoadPhongHat();
        }

        private void LoadPhongHat()
        {
            try
            {
                // Kết nối đến cơ sở dữ liệu PHONGHAT
                string connectionString = "YOUR_CONNECTION_STRING";
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    connection.Open();

                    // Truy vấn để lấy danh sách các phòng đã có sẵn từ cơ sở dữ liệu
                    string query = "SELECT MaPH FROM PHONGHAT";
                    SqlCommand command = new SqlCommand(query, connection);
                    SqlDataReader reader = command.ExecuteReader();

                    // Xóa danh sách phòng cũ
                    comboBoxPhongHat.Items.Clear();

                    // Thêm các phòng vào danh sách
                    while (reader.Read())
                    {
                        string maPH = reader.GetString(0);
                        comboBoxPhongHat.Items.Add(maPH);
                    }
        ...
        private void comboBoxPhongHat_SelectedIndexChanged(object sender, EventArgs e)
        {
            // Lấy giá trị MaPH từ lựa chọn của người dùng
            currentMaPH = comboBoxPhongHat.SelectedItem.ToString();
        }

        private void btnThayDoiMaPH_Click(object sender, EventArgs e)
        {
            // Kiểm tra xem đã chọn phòng hay chưa
            if (string.IsNullOrEmpty(currentMaPH))
            {
                MessageBox.Show("Vui lòng chọn một phòng.");
                return;
            }

            // Thực hiện các thay đổi với giá trị MaPH tại đây
            // Ví dụ: có thể lưu giá trị MaPH vào một biến hoặc truyền giá trị này cho các phương thức khác
            MessageBox.Show("Giá trị MaPH đã được thay đổi thành: " + currentMaPH);
        }
